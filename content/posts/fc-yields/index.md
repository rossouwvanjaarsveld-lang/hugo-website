...
draft: false
summary: >-
  I raced about twenty forecasting models against each other on SA government
  bond yield buckets, and the humble random walk turned out to be really
  difficult to beat. The only thing that reliably helps is averaging two simple
  models. Sophistication is not the same thing as accuracy.
categories:
  - yield curve
  - machine learning
  - fixed income
description: >-
  A leakage-free walk-forward horse race between direct yield forecasting models
  (Ridge, Lasso, ElasticNet, LightGBM, AR(1)) and Nelson-Siegel factor
  approaches (DNS, AFNS) across four SARB government bond maturity buckets and
  six horizons.
...
<figure>
<img src="figures/fig_01_yields.png" style="width:100.0%" alt="South African government bond yields by maturity bucket, monthly 1998-2026. Four lines, two decades of monetary policy, and the small heart-attack of 2020 clearly visible in all four." />
<figcaption aria-hidden="true">South African government bond yields by maturity bucket, monthly 1998-2026. Four lines, two decades of monetary policy, and the small heart-attack of 2020 clearly visible in all four.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_02_acf.png" style="width:100.0%" alt="Autocorrelation function of monthly yields, by tenor. The slow decay is the signature of a near-unit-root process: yields today are almost the same as yields next month, the month after, and so on. The random-walk benchmark is going to be hard to beat at short horizons." />
<figcaption aria-hidden="true">Autocorrelation function of monthly yields, by tenor. The slow decay is the signature of a near-unit-root process: yields today are almost the same as yields next month, the month after, and so on. The random-walk benchmark is going to be hard to beat at short horizons.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_03_forecastability.png" style="width:100.0%" alt="Forecastability profile for the four observed SARB government bond buckets. Sample Entropy (left), Variance Ratio VR(12) (centre, with the random-walk benchmark of 1.0 in red), and ACF(1) (right). All four buckets show ACF(1) ≥ 0.97 and VR(12) close to 1 — near-I(1) dynamics. The short bucket (0-3Y) has slightly higher Sample Entropy, consistent with more volatile short-maturity dynamics relative to the more persistent long end." />
<figcaption aria-hidden="true">Forecastability profile for the four observed SARB government bond buckets. Sample Entropy (left), Variance Ratio VR(12) (centre, with the random-walk benchmark of 1.0 in red), and ACF(1) (right). All four buckets show ACF(1) ≥ 0.97 and VR(12) close to 1 — near-I(1) dynamics. The short bucket (0-3Y) has slightly higher Sample Entropy, consistent with more volatile short-maturity dynamics relative to the more persistent long end.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_04_dist.png" style="width:100.0%" alt="Distribution of monthly yield changes with normal overlay. Fat tails are obvious." />
<figcaption aria-hidden="true">Distribution of monthly yield changes with normal overlay. Fat tails are obvious.</figcaption>
</figure>
...
The Nelson-Siegel decomposition (Nelson and Siegel 1987) writes the yield
curve as
...
**AFNS** (Christensen et al. 2011) adds an arbitrage-free correction: a per-tenor
constant bias
...
recipe (Diebold and Li 2006) uses an AR(1) per factor --- three independent
...
<figure>
<img src="figures/fig_07_rmsse-grid.png" style="width:100.0%" alt="RMSSE per cell across all 8 models. Green = beats naive; red = worse than naive." />
<figcaption aria-hidden="true">RMSSE per cell across all 8 models. Green = beats naive; red = worse than naive.</figcaption>
</figure>

<figure>
<img src="figures/fig_08_da-grid.png" style="width:100.0%" alt="Directional accuracy per cell. Yellow ~ coin flip." />
<figcaption aria-hidden="true">Directional accuracy per cell. Yellow ~ coin flip.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_10_direct-vs-factor.png" style="width:100.0%" alt="Best direct vs best factor model per cell. Negative (green) = direct beats factor; positive (red) = factor beats direct." />
<figcaption aria-hidden="true">Best direct vs best factor model per cell. Negative (green) = direct beats factor; positive (red) = factor beats direct.</figcaption>
</figure>

<figure>
<img src="figures/fig_11_direct-vs-factor-da.png" style="width:100.0%" alt="Same head-to-head as the RMSSE chart above, but using directional accuracy. Each cell shows (best direct DA) − (best factor DA), in percentage points. Positive (green) means the best direct model gets the direction right more often than the best factor model. Note that the DA picture is much messier than RMSSE: many cells are near zero, factor models actually beat direct on DA in several long-horizon cells, and the cross-over pattern is less clean." />
<figcaption aria-hidden="true">Same head-to-head as the RMSSE chart above, but using directional accuracy. Each cell shows (best direct DA) − (best factor DA), in percentage points. Positive (green) means the best direct model gets the direction right more often than the best factor model. Note that the DA picture is much messier than RMSSE: many cells are near zero, factor models actually beat direct on DA in several long-horizon cells, and the cross-over pattern is less clean.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_12_best-per-cell.png" style="width:100.0%" alt="Best model per cell with achieved RMSSE. Five different families share the 24 cells." />
<figcaption aria-hidden="true">Best model per cell with achieved RMSSE. Five different families share the 24 cells.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_13_factor-dynamics.png" style="width:100.0%" alt="Factor dynamics horse race — mean RMSSE across all four buckets, by horizon, for six representative AFNS factor-dynamics variants. AR(1) per factor wins decisively at h=60 (RMSSE 0.87). Random walk per factor is the surprise — competitive at every horizon, best at h=12 and h=24. The ML-on-factors variants (Ridge, ENet, Lasso, LightGBM) all degrade at long horizons despite the rich feature set. The full 8-variant heatmap is in the next chart." />
<figcaption aria-hidden="true">Factor dynamics horse race — mean RMSSE across all four buckets, by horizon, for six representative AFNS factor-dynamics variants. AR(1) per factor wins decisively at h=60 (RMSSE 0.87). Random walk per factor is the surprise — competitive at every horizon, best at h=12 and h=24. The ML-on-factors variants (Ridge, ENet, Lasso, LightGBM) all degrade at long horizons despite the rich feature set. The full 8-variant heatmap is in the next chart.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_14_factor-dynamics-heatmap.png" style="width:100.0%" alt="Per-cell comparison of all eight AFNS factor-dynamics variants. Each cell shows the RMSSE of AFNS with that specific dynamics. AR(1) wins almost every long-horizon cell; RW competes at medium horizons; VAR, BVAR, and all four ML-on-factors variants are substantially worse at h&gt;=24." />
<figcaption aria-hidden="true">Per-cell comparison of all eight AFNS factor-dynamics variants. Each cell shows the RMSSE of AFNS with that specific dynamics. AR(1) wins almost every long-horizon cell; RW competes at medium horizons; VAR, BVAR, and all four ML-on-factors variants are substantially worse at h&gt;=24.</figcaption>
</figure>
...
The "proper" AFNS (Christensen et al. 2011) treats the factors as *latent states*,
...
<figure>
<img src="figures/fig_18_kalman.png" style="width:100.0%" alt="Full state-space AFNS (Kalman + MLE) vs simple two-step DNS-RW, mean RMSSE by horizon. The theoretically-superior model is decisively worse at every horizon. The reasons are structural: (i) it imposes stationary mean-reversion on factors that are near-unit-root, so it over-reverts — exactly the AR(1) failure mode; (ii) the measurement-error filtering it offers is worthless here because four buckets and three factors leave the cross-section almost exactly determined, so there is essentially no measurement noise to filter out." />
<figcaption aria-hidden="true">Full state-space AFNS (Kalman + MLE) vs simple two-step DNS-RW, mean RMSSE by horizon. The theoretically-superior model is decisively worse at every horizon. The reasons are structural: (i) it imposes stationary mean-reversion on factors that are near-unit-root, so it over-reverts — exactly the AR(1) failure mode; (ii) the measurement-error filtering it offers is worthless here because four buckets and three factors leave the cross-section almost exactly determined, so there is essentially no measurement noise to filter out.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_19_convexity.png" style="width:100.0%" alt="The theoretical AFNS convexity term by maturity (left), and the proof that it cancels for forecasting (right). Left: A(τ)/τ rises from a few bp at the short bucket to ~200 bp at 12.5 years. Right: when the convexity term is applied consistently — extract factors from (yields + convexity), then reconstruct as (loadings·factors − convexity) — the forecast is within ~4 bp of plain DNS-RW. The deterministic term enters extraction and reconstruction with opposite signs and almost perfectly cancels." />
<figcaption aria-hidden="true">The theoretical AFNS convexity term by maturity (left), and the proof that it cancels for forecasting (right). Left: A(τ)/τ rises from a few bp at the short bucket to ~200 bp at 12.5 years. Right: when the convexity term is applied consistently — extract factors from (yields + convexity), then reconstruct as (loadings·factors − convexity) — the forecast is within ~4 bp of plain DNS-RW. The deterministic term enters extraction and reconstruction with opposite signs and almost perfectly cancels.</figcaption>
</figure>
...
<figure>
<img src="figures/fig_22_noise-floor.png" style="width:100.0%" alt="Empirical floor vs theoretical AR(1) bound by tenor." />
<figcaption aria-hidden="true">Empirical floor vs theoretical AR(1) bound by tenor.</figcaption>
</figure>
...
Bates-Granger (Bates and Granger 1969) optimal combination on rolling
...
Diebold, Francis X., and Canlin Li. 2006. "Forecasting the Term Structure of Government Bond Yields." *Journal of Econometrics* 130: 337--64.

Nelson, Charles R., and Andrew F. Siegel. 1987. "Parsimonious Modeling of Yield Curves." *Journal of Business* 60 (4): 473--89.
