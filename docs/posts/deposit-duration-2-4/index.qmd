...
summary: "Post 1/4 of the No Gut Feelings Bank series. How much maturity transformation does an SA deposit base license, and is the
...
a licence for maturity transformation &mdash; not an exemption from rate risk:
...
<em>cuts</em> until the structural hedge takes the licence up. Licence size:
&minus;1 to &minus;7 years per rand, on one judgement &mdash; how much of the
...
added &mdash; it <em>is</em> the balance sheet. What it does is <strong>license
...
positive duration. DSS&rsquo;s phrase &mdash; maturity transformation
<em>without</em> interest rate risk &mdash; describes US banks that actually
...
and the bank short rate <em>cuts</em> &mdash; the familiar endowment exposure.
...
ggplot() +
  ...
  labs(x = "uninsured franchise share  u  (by value)",
       ...
       caption = "φ anchored by real duration · u-axis & frontiers still synthetic") +
...
<div style="display:flex;align-items:flex-start;gap:14px;border:1px solid #1d3a55;border-left:4px solid #38bdf8;border-radius:8px;background:#0d1d30;padding:16px 20px;margin:1.5rem 0;font-size:.95rem;line-height:1.55;color:#d9e6f2;">
  ...
  <div>
    ...
    <div>This plane is not a DSSW figure &mdash; it is their capital condition
...
#| echo: false
man <- data.frame(
  field = c("beta_level","alpha","survival","weight","repo","decay_rate_sens"),
  value = c("op 0.46 · notice 0.63 · term 0.25 (cell means)",
            "op 2.58% · notice 2.93% · term 6.89%",
            "cohort 16.4/16.6/24.4 mo; level permanent",
            "0.239 / 0.089 / 0.234 / 0.437",
            "6.75%",
            "prior tight at 0 in-sample; external tail"),
  source = c("results$betas (cleaned names)",
             "d_now − beta·repo from D$RATE tail (EVE-consistent)",
             "results$dur_cohort, results$uc",
             "results$composition$pi_ngfb",
             "results$eve$repo_now",
             "justified by null coupling (corr 0.012); not estimated by Post 1"))
knitr::kable(man, col.names = c("interface field","real value used","Post 1 source"))
```

Savings beta/intercept are proxied on operational (no savings rate series).
